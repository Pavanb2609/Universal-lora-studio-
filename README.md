...
**Windows** — double-click `run.bat`, or from PowerShell:
...
Either way, open <http://127.0.0.1:8000> when it reports it is running.

Both scripts create a virtual environment under `backend/.venv`, install the
dependencies into it, and start the server. To do it by hand instead:
...
python -m uvicorn uls.api.app:app --port 8000
```
...
Detection works the same way there, with a few differences worth knowing.

**Python.** Install from [python.org](https://www.python.org/downloads/) and
tick *Add python.exe to PATH* during setup. The Microsoft Store build also
works. `run.bat` will tell you if it cannot find an interpreter.

**NVIDIA GPUs** are detected through `nvidia-smi`, which the driver installs to
`C:\Windows\System32` and is therefore on PATH already. Nothing extra is
needed for the hardware page to show your card. Installing PyTorch improves the
reading, because torch reports the memory pool a process can actually use,
which differs from the card's nameplate under MIG or a memory cap.

**bitsandbytes** — and therefore QLoRA and 8-bit optimizers — needs a Windows
build. Recent releases ship Windows wheels; if yours does not import, the
studio reports 4-bit as unavailable with that reason and plans standard LoRA
instead rather than producing a configuration that would fail. Many people run
training under WSL2 for this reason, where the Linux wheels apply.

**AMD GPUs on native Windows** are not detected, because ROCm is Linux-only.
The studio will fall back to CPU and say so. If you have an AMD card, WSL2 with
ROCm is the path.

**Firewall.** The server binds to `127.0.0.1`, so Windows Defender should not
prompt. If you change the host to expose it on your network, it will.

**If PowerShell blocks the script**, use `run.bat` rather than a `.ps1` — batch
files are not subject to the execution policy.
